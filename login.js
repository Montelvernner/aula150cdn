class Login{
    ...
    static endpoint="https://loginv1.diegovgianini.repl.co/";//?matricula=123&senha=321";

    static login=(callback_ok,calback_naook,config=null)=>{
        if(config!=null){
            this.config=config;
        }
        ...
        `.botoesLogin button{cursor: pointer;background-color: #${this.config.cor};color: #fff;border-radius: 5px;padding: 10px 40px;box-sizing: inherit;}`
    ...
    static verificaLogin=()=>{
        ...
        const endpoint=`https://loginv1.diegovgianini.repl.co/?matricula=${mat}&senha=${pas}`;
        fetch(endpoint)
        .then(res=>res.json())
        .then(res=>{
            if(res){
